...
class CommandInterpreter(cmd.Cmd):
    ...
    def _pre_command(self, line):
        ...
        match_uid_and_args = re.search('^"([^"]*)"(?:, (.*))?$', arguments)
        ...
        if method_name == "update" and attr_or_dict:
            ...
            if match_attr_and_value:
                attribute_and_value = (match_attr_and_value.group(
                    1) or "") + " " + (match_attr_and_value.group(2) or "")
        command = f"{method_name} {class_name} {uid} {attribute_and_value}"
        self.onecmd(command)
        return command
    ...
    def do_all_instances(self, line):
